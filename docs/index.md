...
入社後は、研修を経て Java のプロジェクトに約 1 ヶ月間参画した後、自社で展開する Web サービスの開発に従事しておりました。

## 保有スキル

- React における One-Way Dataflow (単方向データフロー)に則った SPA 開発
...
| 種類         | 期間   | レベル                                                                                               |
| ------------ | ------ | ---------------------------------------------------------------------------------------------------- |
| React        | 7 ヶ月 | 単一責任の原則に準じたコンポーネント設計・開発が可能<br>ライブラリを使用したキャッシュ管理が可能<br> |
...
| Redux        | 3 ヶ月 | Redux tool kit を用いたグローバルステートの管理が可能                                                |
...
### 株式会社 Crane&I（2022/04/01〜現在）

#### プロジェクト概要（2022/07/01〜現在）

- 内容
  - インフラエンジニア向けオンライン学習サービスの開発  
    Study&Works という、インフラエンジニア向けのオンライン学習サービスの開発に携わりました。具体的には、下記の 3 つのサービスにおける、フロントエンド開発と AWS Amplify を用いたサーバレスバックエンド環境の構築に携わりました。
    - Study Works  
      ユーザが学習するためのページ。Linux や SQL の基礎などが学べる。
    - Study Works Admin(管理者画面)  
      Study Works 利用ユーザの基本情報と進捗が確認できる画面。
    - Study Works Scout  
      Study Works 利用ユーザを法人がスカウトするためのダイレクトリクルーティングサービス。
- 規模
...
#### プロジェクト概要（2022/06/01〜2022/06/31）

- 内容
  - 通信系企業の光回線契約管理システムの改修・テスト  
     システムの仕様変更に伴う、ソースコードの改修(Java)。それに伴う単体テスト・結合テストの実施を行いました。
- 規模
  - 4 名
- 役割
  - テスター
...
- 開発・製造
  - 機能仕様書作成
  - batch 処理ツール製造(ShellScript)
  - エラーハンドリング処理のソースコード改修(Java)
